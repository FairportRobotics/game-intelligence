...
conn = sqlite3.connect("tba-api.db")
cursor = conn.cursor()
comp_levels = {"qm":1, "qf":2, "sf":3, "f":4, "ef":5}
...
cursor.execute(
    'CREATE TABLE "tba_matches" ( id INTEGER PRIMARY KEY, "key" TEXT, "event_key" TEXT, "match_number" INTEGER, "comp_level" TEXT, "set_number" INTEGER, "blue_score" INTEGER, "blue_team_1" TEXT, "blue_team_2" TEXT, "blue_team_3" TEXT, "blue_team_4" TEXT, "red_score" INTEGER, "red_team_1" TEXT, "red_team_2" TEXT, "red_team_3" TEXT, "red_team_4" TEXT )'
)
conn.commit()
...
for file_path in sorted(glob("raw_data/tba_matches/*.json")):
    ...
    with open(file_path, "r") as json_file:
        for temp in json.load(json_file):
            ...
            data.append(row)
data = pd.DataFrame(data).sort_values(by=["event_key", "match_type", "match_number", "set_number"]).drop("match_type", axis=1)
